...
Следуй им, даже если привычка подсказывает иначе: единообразие в кодовой базе дороже, чем каждый отдельный выбор из этого списка.
...
Обратная сторона: не превращай в объединение то, что независимо. Если два поля меняются сами по себе и любая их комбинация допустима, объединение лишнее и только мешает.

```ts
// хорошо: флаги независимы, объединение тут ни к чему
type Panel = {
  isCollapsed: boolean
  isPinned: boolean
}
...
```ts
// плохо: четыре варианта вручную, хотя все комбинации законны
type Panel =
  | { kind: "open-unpinned" }
  | { kind: "open-pinned" }
  | { kind: "collapsed-unpinned" }
  | { kind: "collapsed-pinned" }
```

Признак, по которому выбирают: объединение нужно, когда комбинации *невозможны*. Если возможны все, это просто поля.
