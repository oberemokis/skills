...
Vue 3.5 добавила `useTemplateRef("name")`, где связь идёт по строке, а не по совпадению имени переменной с атрибутом. Форма надёжнее при переименованиях. В файле, где уже используется обычный `ref`, следуй существующему коду: смешивать две формы в одном компоненте хуже, чем выбрать любую из них.
...
```vue
<!-- плохо: накрывающая кнопка объявлена последней и съест клики по стрелке -->
<template>
  <button class="absolute left-3" @click="go(-1)" />
  <button class="absolute inset-0" @click="zoomed = true" />
</template>
...
Диагностика здесь обманчива: ссылка на стрелку живая и валидная, обработчик на месте, а клик не доходит.
